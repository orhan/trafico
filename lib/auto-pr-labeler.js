class AutoLabeler {
  ...
  async runAutoLabeler(context, pullRequest, config) {
    ...
    statuses.forEach(status => {
      switch (status) {
        case AutoLabeler.STATUS.WIP:
          if (this.config.addWipLabel) {
            this._addStatusLabel(status);
          }
          break;
        case AutoLabeler.STATUS.UNREVIEWED:
        case AutoLabeler.STATUS.CHANGES_REQUESTED:
        case AutoLabeler.STATUS.APPROVED:
        case AutoLabeler.STATUS.MERGED:
          this._addStatusLabel(status);
          break;
        default:
  ...
  async _getStatuses() {
    ...
    if (this.pullRequest.title.match(wipRegex)) {
      return [AutoLabeler.STATUS.WIP];
    } else if (this.pullRequest.state === "closed" && this.pullRequest.merged) {
      return [AutoLabeler.STATUS.MERGED];
    }

    const reviews = await this._getUniqueReviews();
    if (reviews.length === 0) {
      return [AutoLabeler.STATUS.UNREVIEWED];
    } else {
      const changeRequestedReviews = reviews.filter(
        review =>
          review.state === AutoLabeler.GH_REVIEW_STATUS.CHANGES_REQUESTED
      );
      const approvedReviews = reviews.filter(
        review => review.state === AutoLabeler.GH_REVIEW_STATUS.APPROVED
      );

      if (changeRequestedReviews.length > 0) {
        return [AutoLabeler.STATUS.CHANGES_REQUESTED];
      } else if (reviews.length === approvedReviews.length) {
        const statuses = [AutoLabeler.STATUS.APPROVED];
        ...
        if (approvedReviews.length < requiredApprovals) {
          status.push(AutoLabeler.STATUS.UNREVIEWED);
        }
  ...
  async _getUniqueReviews() {
    ...
    const uniqueReviews = reviews
      ...
      .filter(
        review =>
          review.state === AutoLabeler.GH_REVIEW_STATUS.APPROVED ||
          review.state === AutoLabeler.GH_REVIEW_STATUS.CHANGES_REQUESTED
      )
  ...
  async _updateLabel(status) {
    if (status !== AutoLabeler.STATUS.WIP || this.config.addWipLabel) {
      this._addStatusLabel(status);
    }
  }
}
